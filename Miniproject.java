//DSA SEM-II MINIPROJECT
//ROLL NOS:2321,2326,2327
//DIV:A
//BATCH:A2
...
class graph              // graph class
{
	...
	void create()    //create graph function
	{
    	...
    	for(int j=0;j<e;j++)
    	{
        	...
        	System.out.print("\nEnter distance  between  "+p.sh[u1].type+" "+u+" and "+p.sh[v1].type+" "+v+" (in Km) : ");
        	flag1=0;
        	int w=0;
	...
	int minimum_vertex(int mst[], int key[])
	{    //function to get minimum vertex for dijkstra's algo
        int minKey = 9999;
        int vertex = -1;
        for (int i = 0; i <n ; i++) 
        {
	...
	void dijkstra(int src,int dest)    //dijkstra Algorithm
	{
         ...
         for (int i = 0; i <n ; i++)
         {
             ...
             //iterate through all the adjacent vertices of above vertex and update the keys
	             for (int vertex_v = 0; vertex_v <n ; vertex_v++)
	             {
	                 ...
	                 {
	                     /*check if this vertex 'vertex_v' is  already visited and
	                      if distance[vertex_V]!=max*/
	
	                     if(visited[vertex_v]==0 && adj[vertex_u][vertex_v]!=9999)
	                     {   /*if not visited and matrix=infinity
	                         check if distance needs an update or not
	                         means check total weight from source to vertex_v < the current distance value, 
         ...
         System.out.println("\n\t   SHORTEST PATH BETWEEN "+p.sh[src].type.toUpperCase()+" "+p.sh[src].shop_house_no+" AND "+p.sh[dest].type.toUpperCase()+" "+p.sh[dest].shop_house_no);
         System.out.println("_____________________________________________________________________________________________________");
	...
	{
		System.out.println("_____________________________________________________________________________________________________");
	    System.out.println("\n\t   DISTANCE OF "+p.sh[src].type.toUpperCase()+" "+p.sh[src].shop_house_no+" TO ALL SHOPS & HOUSES");
	    System.out.println("_____________________________________________________________________________________________________");
	    System.out.println();
	    for (int i = 0; i <n ; i++) 
	    {  if(dist[i]!=9999)
	          System.out.println(p.sh[src].type.toUpperCase()+"  " + p.sh[src].shop_house_no +" ---> "+p.sh[i].type.toUpperCase()+"  "+ p.sh[i].shop_house_no +"  distance is : " + mat[i]+" Km.");
	    }
	...
	{ 
	    ...
	    System.out.print("\n "+p.sh[src].shop_house_no+"-->"+p.sh[dest].shop_house_no +"\t   " +dist[dest]+" Km"+"\t\t  "+p.sh[src].type.toUpperCase()+" "+p.sh[src].shop_house_no); 
	    printPath(pred, dest); 
	    System.out.println("\n");
	...
	System.out.println("\nNearest Shop to "+p.sh[no].type.toUpperCase()+" "+p.sh[no].shop_house_no+" is "+p.sh[p.map(x)].type.toUpperCase()+" "+p.sh[p.map(x)].shop_house_no+ " at a distance of "+p.arr[y].shop_house_no+" Km.");
	System.out.println("_____________________________________________________________________________________");
    }
...
public class Miniproject {
	...
	public static void main(String[] args) {
		...
		do {
			...
			System.out.println("\n*************************************************************************");
			cnt++;
			if(cnt==1)             //validation for compelling user to accept data first before performing any other operations
			{
				while(!(ch==1))
				{
			...
			case 5:
				...
				{    //shopkeeper
					do {
						...
						case 3:System.out.println("OPEN or CLOSE");
					        	flag1=0;
